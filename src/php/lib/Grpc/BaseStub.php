...
class BaseStub
{
     ...
     * @param ClientInterceptor $interceptors An array of client interceptors
     */
    public function __construct($hostname, $opts, Channel $channel = null, array $interceptors = array())
    {
                                   ...
                                   array $options = [])
    {
        ...
        $interceptedCall = array_reduce($this->interceptors, function($c,$curr) use ($method, $argument, $metadata, $options) {
            $curr->interceptUnaryCall($method, $argument, $metadata, $options, $c);
        }, $call);

        $interceptedCall->start($argument, $metadata, $options);
